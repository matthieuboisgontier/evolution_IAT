...
## Tabled with mean age and % of women per year and occupation
summary_table <- summary_table %>%
  mutate(
    Age_SD = paste0(round(mean_age, 1), " (", round(sd_age, 1), ")")
  ) %>%
  select(year, occupation_code, n_total, Age_SD, perc_women)

# Pivot wider
wide_table <- summary_table %>%
  pivot_wider(
    names_from = occupation_code,
    values_from = c(n_total, Age_SD, perc_women),
    names_sep = "_"
  ) %>%
  arrange(year)
...
final_table <- wide_table %>%
  rename(
    Year = year,
    `Clin_n` = n_total_clin,
    `Clin_Age (SD)` = Age_SD_clin,
    `Clin_% women` = perc_women_clin,
    `Rehab_n` = n_total_rehab,
    `Rehab_Age (SD)` = Age_SD_rehab,
    `Rehab_% women` = perc_women_rehab,
    `Other_n` = n_total_other,
    `Other_Age (SD)` = Age_SD_other,
    `Other_% women` = perc_women_other
  )

# Make flextable and export
ft <- flextable(final_table) %>%
  autofit()

doc <- read_docx() %>%
  body_add_par("Table: Mean Age (SD) and % Women by Year and Occupation", style = "heading 1") %>%
